...
export interface CollectedInfo {
  decision_scenario: Record<string, string>;
  constraints: Record<string, string>;
  priorities: Record<string, string>;
  concerns: string[];
  mentioned_options: string[];
}
...
export interface KnowledgeGraphViewNode {
  ...
  type: string;
  category?: string;
  view_role: 'person' | 'education';
  weight: number;
  influence_score: number;
  connections: number;
  insight_tags: string[];
  stories?: string[];  // 人物故事列表
...
export interface KnowledgeGraphViewSummary {
  user_id: string;
  view_mode: 'people' | 'education' | 'career';
  node_count: number;
  link_count: number;
  top_nodes: string[];
}
...
export interface FutureOsContextBrief {
  ...
  career_count: number;
  key_people: string[];
  key_education: string[];
  key_career: string[];
}

export interface FutureOsContext {
  question: string;
  recommended_view: 'people' | 'education' | 'career';
  profile: PersonalizationProfile;
  top_people: KnowledgeGraphViewNode[];
  top_education: KnowledgeGraphViewNode[];
  top_career: KnowledgeGraphViewNode[];
