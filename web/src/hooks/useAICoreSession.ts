...
export function useAICoreSession(userId: string | undefined) {
  ...
  useEffect(() => {
    ...
    const initSession = async () => {
      // 1. 尝试从localStorage恢复会话
      const savedSessionId = localStorage.getItem(AI_CORE_SESSION_KEY);
      const savedMessages = localStorage.getItem(AI_CORE_MESSAGES_KEY);

      if (savedSessionId && savedMessages) {
        try {
          const parsedMessages = JSON.parse(savedMessages);
          globalSessionId = savedSessionId;
          globalMessages = parsedMessages;
          setSessionId(savedSessionId);
          setMessages(parsedMessages);
          console.log('[AI核心] 恢复会话:', savedSessionId, '消息数:', parsedMessages.length);
          return;
        } catch (e) {
          console.error('[AI核心] 恢复会话失败:', e);
        }
      }
      ...
      try {
        ...
        console.log('[AI核心] 创建新会话:', newSessionId);
      } catch (err) {
