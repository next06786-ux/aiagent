...
export function normalizeDecisionGraphNode(
  ...
  // console.log('[normalizeDecisionGraphNode] 输入event_id:', event.event_id, '输出event_id:', normalized.event_id, 'parent:', normalized.parent_event_id);
  
  return normalized;
}

export function buildDecisionGraphEdges(nodes: DecisionGraphNode[]): DecisionGraphEdge[] {
  // console.log('[buildDecisionGraphEdges] 开始构建边，节点数量:', nodes.length);
  
  const edges = nodes.reduce<DecisionGraphEdge[]>((result, node) => {
    if (!node.parent_event_id) {
      // console.log(`[buildDecisionGraphEdges] 跳过根节点: ${node.event_id}`);
      return result;
    }
    ...
    // console.log(`[buildDecisionGraphEdges] 创建边: ${edge.source} -> ${edge.target}`);
    result.push(edge);

    return result;
  }, []);
  
  // console.log('[buildDecisionGraphEdges] 完成，边数量:', edges.length);
  return edges;
}
...
export function buildFallbackDecisionGraph(
  ...
  // 移除频繁的日志输出，避免性能问题
  // console.log('[buildFallbackDecisionGraph] 开始构建，optionId:', optionId, '节点数:', timeline.length);
