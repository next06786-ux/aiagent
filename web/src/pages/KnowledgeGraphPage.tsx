...
import { getFutureOsGraphView, getCareerGraphView, getEducationGraphView, getPeopleGraphView } from '../services/futureOs';
import { API_BASE_URL } from '../services/api';
import type { KnowledgeGraphView, KnowledgeGraphViewNode } from '../types/api';

type GraphViewMode = 'people' | 'education' | 'career';
...
function nodeColor(node: KnowledgeGraphViewNode, mode: GraphViewMode): number {
  ...
  const map = mode === 'people' ? PEOPLE_COLORS : mode === 'education' ? EDUCATION_COLORS : CAREER_COLORS
...
function buildGraphInScene(
  ...
  console.log(`[KG] Links processing: total=${data.links.length}, valid edges=${fe.length}`);
  if (data.links.length > 0 && fe.length === 0) {
    console.warn('[KG] ⚠️ All links were filtered out! Sample link:', data.links[0]);
    console.warn('[KG] ⚠️ Sample node IDs:', Array.from(idxMap.keys()).slice(0, 5));
  }
  ...
  fe.forEach(([a, b], edgeIndex) => {
    ...
    if (mode === 'career') {
      ...
      if (linkType === 'mastery') {
        // 我→技能：统一使用蓝色，根据掌握度调整透明度和粗细
        const weight = (link as any)?.weight || 0.5;
        lineColor = 0x2196F3;  // 统一蓝色
        lineWidth = 1 + weight * 2;
        opacity = 0.3 + weight * 0.4;  // 0.3-0.7
...
export default function KnowledgeGraphPage() {
  ...
  useEffect(() => {
    ...
    // 生成缓存key（包含用户ID以隔离不同用户的数据）
    const cacheKey = `${user.user_id}-${viewMode}-${question}`;
    ...
    if (viewMode === 'career') {
      ...
      getCareerGraphView({
        ...
        .catch(e => {
          ...
          setGraph(null);
          setError(formatError('career-graph', e instanceof Error ? e.message : '加载失败'));
        })
        .finally(() => {
    ...
    // 升学规划视图使用专门的education-graph API
    if (viewMode === 'education') {
      console.log('[KG] 🚀 发起升学规划视图请求', {
        发起时间: new Date().toLocaleTimeString(),
      });
      
      const requestStartTime = performance.now();
      
      // 使用用户ID生成不同的学术数据（临时方案，未来应从后端查询）
      const userIdHash = user.user_id.split('-')[0];
      const isFirstUser = userIdHash.charCodeAt(0) % 2 === 0;
      
      getEducationGraphView({
        user_id: user.user_id,
        gpa: isFirstUser ? 3.5 : 3.2,
        gpa_max: 4.0,
        ranking_percent: isFirstUser ? 0.2 : 0.35,
        sat_act: isFirstUser ? 1450 : 1350,
        research_experience: isFirstUser ? 0.6 : 0.4,
        publications: isFirstUser ? 2 : 1,
        target_major: isFirstUser ? '计算机科学' : '软件工程',
        target_level: 'master',
        ...
        .catch(e => {
          const errorTime = performance.now() - loadStartTime;
          console.error('[KG] ❌ 升学规划视图加载失败', {
            错误: e instanceof Error ? e.message : String(e),
            耗时: `${errorTime.toFixed(2)}ms`,
          });
          setGraph(null);
          setError(formatError('education-graph', e instanceof Error ? e.message : '加载失败'));
        })
        .finally(() => {
          const finalTime = performance.now() - effectStartTime;
          console.log('[KG] 🏁 升学规划视图Effect完成', {
            总耗时: `${finalTime.toFixed(2)}ms`,
          });
          setIsLoading(false);
        });
    ...
    // 人物关系视图使用专门的people-graph API
    console.log('[KG] 🚀 发起人物关系视图请求', {
      viewMode,
      question: question || '(空)',
      发起时间: new Date().toLocaleTimeString(),
    });
    ...
    getPeopleGraphView({
      ...
      .then(payload => {
        ...
        // 调试：检查节点是否包含stories
        const nodesWithStories = payload.nodes.filter(n => (n as any).stories && (n as any).stories.length > 0);
        console.log('[KG] 📖 包含stories的节点数:', nodesWithStories.length);
        if (nodesWithStories.length > 0) {
          console.log('[KG] 📖 示例节点stories:', {
            name: nodesWithStories[0].name,
            stories: (nodesWithStories[0] as any).stories,
            description: (nodesWithStories[0] as any).description,
            metadata: (nodesWithStories[0] as any).metadata
          });
        }
      ...
      .catch(e => {
        const errorTime = performance.now() - loadStartTime;
        console.error('[KG] ❌ 人物关系视图加载失败', {
          错误: e instanceof Error ? e.message : String(e),
          耗时: `${errorTime.toFixed(2)}ms`,
        });
        setGraph(null);
        setError(formatError('people-graph', e instanceof Error ? e.message : '加载失败'));
      })
      .finally(() => {
        const finalTime = performance.now() - effectStartTime;
        console.log('[KG] 🏁 人物关系视图Effect完成', {
          总耗时: `${finalTime.toFixed(2)}ms`,
        });
        setIsLoading(false);
      });
        ...
        if (hits.length > 0) {
          ...
          if (nm) {
            ...
            console.log('[KG] 🎯 选中节点:', {
              name: nm.node.name,
              id: nm.node.id,
              stories: (nm.node as any).stories,
              description: (nm.node as any).description,
              完整节点: nm.node
            });
    ...
    <AppShell>
      ...
      <div style={{ position: 'relative', height: 'calc(100vh - 58px)', minHeight: 500, borderRadius: 16, overflow: 'hidden', margin: '-28px' }}>
          ...
          {isLoading && (
            <div style={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center', background: 'rgba(6,13,26,0.8)', backdropFilter: 'blur(8px)', zIndex: 5 }}>
              <div style={{ textAlign: 'center', color: 'rgba(147,197,253,0.9)' }}>
                ...
                <div style={{ fontSize: 12, color: 'rgba(147,197,253,0.6)' }}>正在构建{viewMode === 'people' ? '人物关系' : viewMode === 'education' ? '升学规划' : '职业发展'}图谱</div>
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{
          ...
          >人物关系</button>
          <button
            className={`button ${viewMode === 'education' ? 'button-primary' : 'button-ghost'}`}
            style={{ padding: '6px 14px', fontSize: 13, transition: 'all 0.2s ease' }}
            onClick={() => handleViewModeChange('education')}
            disabled={isLoading}
          >升学规划</button>
          <button
        ...
        {viewMode === 'career' && graph && showLegend && (
          <div style={{
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: 'rgba(147,197,253,0.9)', marginBottom: 10 }}>
                节点类型
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 16, height: 16, borderRadius: '50%', background: '#e8f4ff', flexShrink: 0 }} />
                  <span style={{ fontSize: 12, color: 'rgba(147,197,253,0.8)' }}>中心节点（我）</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 16, height: 16, borderRadius: '50%', background: '#4CAF50', flexShrink: 0 }} />
                  <span style={{ fontSize: 12, color: 'rgba(147,197,253,0.8)' }}>已掌握技能</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 16, height: 16, borderRadius: '50%', background: '#FFC107', flexShrink: 0 }} />
                  <span style={{ fontSize: 12, color: 'rgba(147,197,253,0.8)' }}>部分掌握技能</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 16, height: 16, borderRadius: '50%', background: '#F44336', flexShrink: 0 }} />
                  <span style={{ fontSize: 12, color: 'rgba(147,197,253,0.8)' }}>待学习技能</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 16, height: 16, borderRadius: '50%', background: '#2196F3', flexShrink: 0 }} />
                  <span style={{ fontSize: 12, color: 'rgba(147,197,253,0.8)' }}>岗位</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 16, height: 16, borderRadius: '50%', background: '#9C27B0', flexShrink: 0 }} />
                  <span style={{ fontSize: 12, color: 'rgba(147,197,253,0.8)' }}>公司</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 24, height: 2, background: '#2196F3', flexShrink: 0 }} />
                  <span style={{ fontSize: 12, color: 'rgba(147,197,253,0.8)' }}>技能掌握</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 24, height: 2, background: '#4CAF50', flexShrink: 0 }} />
                  <span style={{ fontSize: 12, color: 'rgba(147,197,253,0.8)' }}>技能要求</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ 
                    width: 24, height: 2, 
                    background: 'repeating-linear-gradient(90deg, #9E9E9E 0, #9E9E9E 4px, transparent 4px, transparent 8px)',
                    flexShrink: 0 
                  }} />
                  <span style={{ fontSize: 12, color: 'rgba(147,197,253,0.8)' }}>所属公司</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ 
                    width: 24, height: 2, 
                    background: 'repeating-linear-gradient(90deg, #FFC107 0, #FFC107 4px, transparent 4px, transparent 8px)',
                    flexShrink: 0 
                  }} />
                  <span style={{ fontSize: 12, color: 'rgba(147,197,253,0.8)' }}>技能依赖</span>
                </div>
              </div>
            </div>

            {/* 层级结构 */}
            {(graph as any).metadata?.layer_info && (
              <div style={{ marginTop: 20, paddingTop: 20, borderTop: '1px solid rgba(99,179,237,0.15)' }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: 'rgba(147,197,253,0.9)', marginBottom: 10 }}>
                  层级结构
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {Object.entries((graph as any).metadata.layer_info).map(([key, info]: [string, any]) => (
                    <div key={key} style={{ fontSize: 12, color: 'rgba(147,197,253,0.7)' }}>
                      <span style={{ color: 'rgba(147,197,253,0.9)' }}>{info.name}</span>
                      {' · '}
                      半径 {info.radius} · {info.count} 个节点
                    </div>
                  ))}
                </div>
              </div>
        ...
        {selectedNode && (
          <div style={{
            ...
            <button
              onClick={async () => {
                if (!user?.user_id || !selectedNode) return;
                
                const confirmDelete = window.confirm(`确定要删除节点"${selectedNode.name}"吗？\n\n此操作将删除该节点及其所有关系，且无法撤销。`);
                if (!confirmDelete) return;
                
                try {
                  const { deleteKnowledgeGraphNode } = await import('../services/futureOs');
                  await deleteKnowledgeGraphNode({
                    user_id: user.user_id,
                  ...
                  console.error('[KG] 删除节点失败:', error);
                  alert(`删除失败: ${error instanceof Error ? error.message : '未知错误'}`);
                }
              }}
              style={{
                position: 'absolute', top: 12, right: 52,
                background: 'rgba(244,67,54,0.15)', border: 'none',
                borderRadius: 8, width: 32, height: 32, cursor: 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                color: 'rgba(244,67,54,0.8)', fontSize: 16,
                transition: 'all 0.2s ease',
              }}
              onMouseEnter={(e) => {
                e.currentTarget.style.background = 'rgba(244,67,54,0.3)';
                e.currentTarget.style.color = 'rgba(244,67,54,1)';
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.background = 'rgba(244,67,54,0.15)';
                e.currentTarget.style.color = 'rgba(244,67,54,0.8)';
              }}
              title="删除节点"
            >🗑</button>
              ...
              <div>
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ fontSize: 13, color: 'rgba(147,197,253,0.7)', lineHeight: 1.6 }}>
                    {(() => {
                      const stories = (selectedNode as any).stories || [];
                      const description = (selectedNode as any).description || '';
                      
                      if (stories.length === 0 && !description) {
                        return (
                        ...
                        <>
                          {description && (
                            <div style={{ background: 'rgba(99,179,237,0.08)', borderRadius: 10, padding: 12, marginBottom: 8 }}>
                              <div style={{ color: '#e8f0fe', marginBottom: 4 }}>{description}</div>
                            </div>
                          )}
                          {stories.map((story: string, idx: number) => (
                            story && story !== description && (
                              <div key={idx} style={{ background: 'rgba(99,179,237,0.08)', borderRadius: 10, padding: 12, marginBottom: 8 }}>
                                <div style={{ color: '#e8f0fe', marginBottom: 4 }}>{story}</div>
                              </div>
                            )
                          ))}
                        </>
                ...
                {graph && (() => {
                  // 获取与当前节点相关的所有连接
                  const relatedNodes: Array<{ node: KnowledgeGraphViewNode; relation: string; story?: string }> = [];
                  ...
                  graph.links.forEach(link => {
                    if (link.source === selectedNode.id) {
                      ...
                      if (targetNode) {
                        // 根据人物关系添加故事
                        let story = '';
                        if (targetNode.name === '李华') story = '大学同学，一起找工作';
                        else if (targetNode.name === '王教授') story = '建议考研，认为就业形势不好';
                        else if (targetNode.name === '女朋友') story = '在上海工作，希望我也去上海';
                        else if (targetNode.name === '父亲') story = '希望我去上海，说北京太远';
                        else if (targetNode.name === '母亲') story = '担心北京生活成本太高';
                        
                        relatedNodes.push({ 
                          node: targetNode, 
                          relation: link.type || 'related',
                          story
                        });
                      }
                      ...
                      if (sourceNode) {
                        let story = '';
                        if (sourceNode.name === '李华') story = '大学同学，一起找工作';
                        else if (sourceNode.name === '王教授') story = '建议考研，认为就业形势不好';
                        else if (sourceNode.name === '女朋友') story = '在上海工作，希望我也去上海';
                        else if (sourceNode.name === '父亲') story = '希望我去上海，说北京太远';
                        else if (sourceNode.name === '母亲') story = '担心北京生活成本太高';
                  ...
                  // 相机跳转函数（复用职业视图的逻辑）
                  const navigateToNode = (targetNode: KnowledgeGraphViewNode) => {
                    const ref = threeRef.current;
                    if (!ref || !ref.camera || !ref.controls) return;

                    const targetMeta = ref.nodes.find(n => n.id === targetNode.id);
                    if (!targetMeta) return;

                    const targetPos = targetMeta.mesh.position.clone();
                    const currentDistance = ref.camera.position.distanceTo(ref.controls.target);
                    const direction = new THREE.Vector3()
                      .subVectors(ref.camera.position, ref.controls.target)
                      .normalize();
                    const newCameraPos = new THREE.Vector3()
                      .copy(targetPos)
                      .add(direction.multiplyScalar(currentDistance));

                    const startCameraPos = ref.camera.position.clone();
                    const startTarget = ref.controls.target.clone();
                    const duration = 1000;
                    const startTime = performance.now();
                    ...
                    const animate = () => {
                      ...
                      ref.camera.position.lerpVectors(startCameraPos, newCameraPos, eased);
                      ref.controls.target.lerpVectors(startTarget, targetPos, eased);
                      ref.controls.update();
                  ...
                  return relatedNodes.length > 0 ? (
                    <div style={{ marginTop: 20, paddingTop: 20, borderTop: '1px solid rgba(99,179,237,0.15)' }}>
                      <div style={{ fontSize: 14, fontWeight: 600, color: 'rgba(147,197,253,0.9)', marginBottom: 12 }}>
                        关联人物 ({relatedNodes.length})
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {relatedNodes.map(({ node, story }, idx) => (
                          <button
                            key={idx}
                          ...
                          >
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: story ? 6 : 0 }}>
                            ...
                            {story && (
                              <div style={{ fontSize: 12, color: 'rgba(147,197,253,0.7)', lineHeight: 1.4 }}>
                                {story}
                              </div>
                            )}
                          </button>
            ...
            {viewMode === 'education' && (
              <div>
                ...
                {/* 院校基本信息 */}
                {selectedNode.type === 'school' && (selectedNode as any).metadata && (() => {
                  const meta = (selectedNode as any).metadata;
                  const matchCategory = meta.match_category;
                  const matchScore = meta.match_score;
                    ...
                    <div style={{ marginBottom: 20 }}>
                      ...
                      {matchScore !== undefined && (
                        <div style={{ marginBottom: 16 }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                            <span style={{ fontSize: 14, fontWeight: 600, color: 'rgba(147,197,253,0.9)' }}>
                              匹配度
                            </span>
                            <span style={{ 
                              fontSize: 13, 
                              fontWeight: 600,
                              color: categoryColors[matchCategory] || '#e8f0fe',
                              padding: '2px 8px',
                              borderRadius: 4,
                              background: `${categoryColors[matchCategory] || '#e8f0fe'}20`,
                            }}>
                              {categoryLabels[matchCategory] || matchCategory}
                            </span>
                          </div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            <div style={{ flex: 1, height: 8, background: 'rgba(99,179,237,0.15)', borderRadius: 4, overflow: 'hidden' }}>
                              <div style={{
                                ...
                                height: '100%',
                                background: `linear-gradient(90deg, ${categoryColors[matchCategory] || '#1890FF'}, ${categoryColors[matchCategory] || '#1890FF'}dd)`,
                                transition: 'width 0.3s ease',
                              }} />
                            </div>
                            <span style={{ fontSize: 13, color: '#e8f0fe', minWidth: 40 }}>
                      ...
                      {meta.match_reasons && meta.match_reasons.length > 0 && (
                        <div style={{ marginBottom: 16 }}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                            {meta.match_reasons.map((reason: string, idx: number) => (
                              <div key={idx} style={{ 
                                background: 'rgba(99,179,237,0.08)', 
                                borderRadius: 8, 
                                padding: '8px 12px',
                      ...
                      <div style={{ background: 'rgba(99,179,237,0.08)', borderRadius: 10, padding: 12, marginBottom: 12 }}>
                        {meta.location && (
                          <div style={{ marginBottom: 8, fontSize: 13 }}>
                            <div style={{ color: 'rgba(147,197,253,0.6)', marginBottom: 2 }}>所在地区</div>
                            <div style={{ color: '#e8f0fe' }}>{meta.location}</div>
                          </div>
                        )}
                        {meta.tier && (
                          <div style={{ marginBottom: 8, fontSize: 13 }}>
                            <div style={{ color: 'rgba(147,197,253,0.6)', marginBottom: 2 }}>院校层次</div>
                            <div style={{ color: '#e8f0fe' }}>{meta.tier}</div>
                          </div>
                        )}
                        {meta.category && (
                          <div style={{ marginBottom: 8, fontSize: 13 }}>
                            <div style={{ color: 'rgba(147,197,253,0.6)', marginBottom: 2 }}>院校类别</div>
                            <div style={{ color: '#e8f0fe' }}>{meta.category}</div>
                          </div>
                        )}
                        {meta.gpa_requirement !== undefined && (
                          <div style={{ marginBottom: 8, fontSize: 13 }}>
                            <div style={{ color: 'rgba(147,197,253,0.6)', marginBottom: 2 }}>GPA要求</div>
                            <div style={{ color: '#e8f0fe' }}>{meta.gpa_requirement}/4.0</div>
                          </div>
                        )}
                        {meta.exam_score_requirement !== undefined && (
                          <div style={{ marginBottom: 8, fontSize: 13 }}>
                            <div style={{ color: 'rgba(147,197,253,0.6)', marginBottom: 2 }}>考研分数要求</div>
                            <div style={{ color: '#e8f0fe' }}>{meta.exam_score_requirement}/500</div>
                          </div>
                        )}
                        {meta.employment_rate !== undefined && (
                          <div style={{ fontSize: 13 }}>
                            <div style={{ color: 'rgba(147,197,253,0.6)', marginBottom: 2 }}>就业率</div>
                            <div style={{ color: '#e8f0fe' }}>{(meta.employment_rate * 100).toFixed(1)}%</div>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                {/* 开设专业列表 */}
                {selectedNode.type === 'school' && (selectedNode as any).metadata?.majors && 
                 (selectedNode as any).metadata.majors.length > 0 && (
                  <div style={{ marginTop: 16 }}>
                    <div style={{ fontSize: 14, fontWeight: 600, color: 'rgba(147,197,253,0.9)', marginBottom: 8 }}>
                      开设专业 ({(selectedNode as any).metadata.majors.length})
                    </div>
                    <div style={{ 
                      maxHeight: 200, 
                      overflowY: 'auto',
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                        {(selectedNode as any).metadata.majors.map((major: string, i: number) => (
                          <span key={i} style={{
                            padding: '4px 10px', 
                            borderRadius: 6, 
                            fontSize: 12,
                ...
                {/* 知名教授列表 */}
                {selectedNode.type === 'school' && (selectedNode as any).metadata?.professors && 
                 (selectedNode as any).metadata.professors.length > 0 && (
                  <div style={{ marginTop: 16 }}>
                    <div style={{ fontSize: 14, fontWeight: 600, color: 'rgba(147,197,253,0.9)', marginBottom: 8 }}>
                      知名教授 ({(selectedNode as any).metadata.professors.length})
                    </div>
                    <div style={{ 
                      maxHeight: 250, 
                      overflowY: 'auto',
                      ...
                      {(selectedNode as any).metadata.professors.map((prof: any, idx: number) => (
                        <div key={idx} style={{
                          background: 'rgba(99,179,237,0.08)',
                          border: '1px solid rgba(99,179,237,0.15)',
                          borderRadius: 8,
                          ...
                          <div style={{ fontSize: 13, fontWeight: 600, color: '#e8f0fe', marginBottom: 4 }}>
                            {prof.name}
                          </div>
                          {prof.research_area && (
                            <div style={{ fontSize: 12, color: 'rgba(147,197,253,0.7)', marginBottom: 2 }}>
                              研究方向: {prof.research_area}
                            </div>
                          )}
                          {prof.h_index !== undefined && (
                            <div style={{ fontSize: 12, color: 'rgba(147,197,253,0.6)' }}>
                              H-index: {prof.h_index}
                            </div>
                          )}
                        </div>
                      ))}
                ...
                {/* 关联节点导航（升学规划视图） */}
                {graph && (() => {
                  const relatedNodes: Array<{ node: KnowledgeGraphViewNode; relation: string }> = [];
                  graph.links.forEach(link => {
                    if (link.source === selectedNode.id) {
                      const targetNode = graph.nodes.find(n => n.id === link.target);
                      if (targetNode) relatedNodes.push({ node: targetNode, relation: link.type || 'related' });
                    } else if (link.target === selectedNode.id) {
                      const sourceNode = graph.nodes.find(n => n.id === link.source);
                      if (sourceNode) relatedNodes.push({ node: sourceNode, relation: link.type || 'related' });
                    }
                  });
                  const groupedNodes: Record<string, KnowledgeGraphViewNode[]> = {};
                  relatedNodes.forEach(({ node, relation }) => {
                    if (!groupedNodes[relation]) groupedNodes[relation] = [];
                    groupedNodes[relation].push(node);
                  });
                  const relationLabels: Record<string, string> = { 'has_achievement': '学业成就', 'qualifies_for': '符合条件的院校', 'requires_action': '需要的行动' };
                  const navigateToNode = (targetNode: KnowledgeGraphViewNode) => {
                    const ref = threeRef.current;
                    if (!ref || !ref.camera || !ref.controls) return;
                    const targetMeta = ref.nodes.find(n => n.id === targetNode.id);
                    ...
                    const targetPos = targetMeta.mesh.position.clone();
                    const currentDistance = ref.camera.position.distanceTo(ref.controls.target);
                    const direction = new THREE.Vector3().subVectors(ref.camera.position, ref.controls.target).normalize();
                    const newCameraPos = new THREE.Vector3().copy(targetPos).add(direction.multiplyScalar(currentDistance));
                    const startCameraPos = ref.camera.position.clone(), startTarget = ref.controls.target.clone();
                    ...
                    const animate = () => {
                      ...
                      ref.controls.target.lerpVectors(startTarget, targetPos, eased);
                      ref.controls.update();
                      if (progress < 1) requestAnimationFrame(animate); else setSelectedNode(targetNode);
                    };
                    animate();
                  };
                  return Object.keys(groupedNodes).length > 0 ? (
                    <div style={{ marginTop: 20, paddingTop: 20, borderTop: '1px solid rgba(99,179,237,0.15)' }}>
                      <div style={{ fontSize: 14, fontWeight: 600, color: 'rgba(147,197,253,0.9)', marginBottom: 12 }}>关联节点</div>
                      {Object.entries(groupedNodes).map(([relation, nodes]) => (
                        <div key={relation} style={{ marginBottom: 16 }}>
                          <div style={{ fontSize: 12, color: 'rgba(147,197,253,0.7)', marginBottom: 8, fontWeight: 600 }}>
                            {relationLabels[relation] || relation} ({nodes.length})
                          </div>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                            {nodes.map((node, idx) => (
                              <button key={idx} onClick={() => navigateToNode(node)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', background: 'rgba(99,179,237,0.08)', border: '1px solid rgba(99,179,237,0.15)', borderRadius: 8, cursor: 'pointer', transition: 'all 0.2s ease', color: '#e8f0fe', fontSize: 13 }} onMouseEnter={(e) => { e.currentTarget.style.background = 'rgba(99,179,237,0.15)'; e.currentTarget.style.borderColor = 'rgba(99,179,237,0.3)'; }} onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(99,179,237,0.08)'; e.currentTarget.style.borderColor = 'rgba(99,179,237,0.15)'; }}>
                                <span style={{ flex: 1, textAlign: 'left', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{node.name}</span>
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, marginLeft: 8, opacity: 0.6 }}><line x1="5" y1="12" x2="19" y2="12" /><polyline points="12 5 19 12 12 19" /></svg>
                              </button>
                            ))}
                          </div>
                        </div>
            ...
            {viewMode === 'career' && (
              <div>
                ...
                {graph && (() => {
                  // 获取与当前节点相关的所有连接
                  const relatedNodes: Array<{ node: KnowledgeGraphViewNode; relation: string; direction: 'in' | 'out' }> = [];
                  ...
                  // 按关系类型分组
                  const groupedNodes: Record<string, Array<{ node: KnowledgeGraphViewNode; direction: 'in' | 'out' }>> = {};
                  relatedNodes.forEach(({ node, relation, direction }) => {
                    if (!groupedNodes[relation]) groupedNodes[relation] = [];
                    groupedNodes[relation].push({ node, direction });
                  });
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                            {nodes.map(({ node, direction }, idx) => (
                              ...
                              >
                                ...
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, marginLeft: 8, opacity: 0.6 }}>
                                  <line x1="5" y1="12" x2="19" y2="12" />
                                  <polyline points="12 5 19 12 12 19" />
                                </svg>
                              </button>
