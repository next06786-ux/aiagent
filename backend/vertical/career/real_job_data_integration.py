...
class RealJobDataIntegration:
    ...
    def __init__(self):
        ...
        # 集成真实数据缓存加载器（优先级最高）
        try:
            from backend.vertical.career.real_job_cache_loader import real_job_cache_loader
            self.cache_loader = real_job_cache_loader
            logger.info("✓ 真实数据缓存加载器已集成")
        except Exception as e:
            logger.warning(f"真实数据缓存加载器加载失败: {e}")
    ...
    def search_jobs(
        ...
        limit: int = 20,
        use_cache: bool = True,
        force_real_data: bool = True  # 新增：强制使用真实数据
    ) -> List[JobData]:
        """
        搜索岗位（多数据源聚合）
        ...
        3. BOSS直聘直接API
        4. 拉勾网API
        5. 仅在force_real_data=False时使用模拟数据
        """
        # 0. 优先尝试从预缓存的真实数据加载
        if self.cache_loader:
            try:
                cached_real_jobs = self.cache_loader.load_jobs(keyword, location, limit)
                if cached_real_jobs:
                    # 转换为JobData对象
                    jobs = []
                    for job_dict in cached_real_jobs:
                        jobs.append(JobData(
                            ...
                            salary_avg=job_dict.get('salary_avg', 0),
                            required_skills=job_dict.get('required_skills', []),
                            experience_required=job_dict.get('experience_required', 0),
                            education_required=job_dict.get('education_required', ''),
                            location=job_dict.get('location', location),
                            district=job_dict.get('district', ''),
                            industry=job_dict.get('industry', ''),
                            company_size=job_dict.get('company_size', ''),
                            company_stage=job_dict.get('company_stage', ''),
                            job_description=job_dict.get('job_description', ''),
                            welfare_tags=job_dict.get('welfare_tags', []),
                            source='cached_real_data',  # 标记为缓存的真实数据
                            source_url=job_dict.get('source_url', ''),
                            crawled_at=job_dict.get('crawled_at', datetime.now().isoformat())
                        ))
                    
                    logger.info(f"✓ 从预缓存加载 {len(jobs)} 个真实岗位数据")
                    return jobs[:limit]
            except Exception as e:
                logger.warning(f"从预缓存加载失败: {e}")
        
        # 1. 尝试内存缓存（仅在不强制真实数据时）
        if use_cache and not force_real_data:
            cached_jobs = self.cache.get(keyword, location, "aggregated")
            if cached_jobs:
                logger.info(f"从内存缓存返回 {len(cached_jobs)} 个岗位")
        ...
        if self.github_crawler and self.github_crawler.available:
            ...
            logger.warning("GitHub爬虫不可用")
        
        # 2. BOSS直聘直接API（备用）
        if len(all_jobs) < limit:
            try:
                logger.info(f"尝试使用BOSS直聘API...")
        ...
        # 5. 如果没有真实数据且不强制真实数据，使用模拟数据
        if not unique_jobs and not force_real_data:
            logger.warning("所有真实数据源失败，使用模拟数据")
            unique_jobs = self._generate_mock_data(keyword, location, limit)
        elif not unique_jobs and force_real_data:
            logger.error("无法获取真实数据，且已禁用模拟数据")
            return []
        ...
        logger.info(f"最终返回 {len(unique_jobs)} 个岗位")
        return unique_jobs[:limit]
    
    def _generate_mock_data(self, keyword: str, location: str, limit: int) -> List[JobData]:
        """生成模拟数据（降级方案）"""
        logger.info(f"生成模拟数据: {keyword} @ {location}")
        
        # 使用现有的拉勾爬虫的模拟数据生成逻辑
        if self.lagou_crawler:
            mock_jobs = self.lagou_crawler._generate_mock_jobs(keyword, location, limit)
            # 转换格式
            return [JobData(
                job_id=job.job_id,
                title=job.title,
