...
class TimelineFeedbackRequest(BaseModel):
    ...
    option_id: str
    event_id: str
    feedback_type: str  # "accurate", "inaccurate", "missing_context"
    feedback_text: str = ""
...
async def get_decision_record(simulation_id: str):
    ...
    try:
        ...
        if not row:
            raise HTTPException(status_code=404, detail="未找到对应的推演记录")
        
        timeline_data = json.loads(row[5]) if row[5] else {}
        return {
            "success": True,
            "data": {
                "simulation_id": row[0],
...
async def simulate_with_collected_info(request: SimulateWithCollectedInfoRequest) -> Dict[str, Any]:
    ...
    此REST API端点已废弃，请使用 /ws/decision-simulate WebSocket端点
    """
    try:
        ...
        # 2. 使用WebSocket推演（推荐方式）
        # 此REST API端点已废弃，请使用 /ws/decision-simulate WebSocket端点
        raise HTTPException(
            status_code=410, 
            detail="此API已废弃，请使用WebSocket推演: /ws/decision-simulate"
        )
    except HTTPException:
        raise
    except Exception as e:
...
"""
            ...
            # ========== 升学教育决策使用多Agent框架（流式推演）==========
                ...
                logger.info(f"🎯 检测到升学教育决策类型，使用多Agent评估框架（实时流式推演），选项数量={len(options)}")
                try:
                    logger.info(f"[升学推演] 开始发送start消息，ws_connected={ws_connected}")
                    if not await safe_send({
                        "type": "start",
                        "session_id": session_id,
                        "user_id": user_id,
                    ...
                    logger.info("[升学推演] start消息发送成功，准备发送status消息")
                    if not await safe_send({
                        "type": "status",
                        "stage": "init",
                        "content": "正在初始化升学教育决策引擎..."
                    }):
                        logger.warning("[升学推演] 发送status失败，连接已断开")
                        return

                    logger.info("[升学推演] 开始导入教育决策模块")

                    logger.info("[升学推演] 开始导入教育决策模块")
                    # 导入教育决策引擎和多Agent评估器
                    from backend.vertical.education.education_decision_engine import (
                        EducationDecisionEngine, EducationDecisionContext, TargetSchool,
                        SchoolType, AcademicProfile, EducationLevel
                    )
                    from backend.decision.multi_agent_education_evaluator import (
                        MultiAgentEducationEvaluator
                    )
                    logger.info("[升学推演] 教育决策模块导入成功")

                    # 初始化教育决策引擎
                    edu_engine = EducationDecisionEngine()
                    logger.info("[升学推演] 教育决策引擎初始化完成")
                    ...
                    # 从collected_info中提取学业信息
                    def extract_academic_info(collected_info: dict, option_title: str) -> tuple:
                        """从收集的信息中提取学业相关数据"""
                        # 尝试从collected_info中提取GPA等信息
                        context_info = collected_info.get('decision_context', {})
                        constraints = collected_info.get('user_constraints', {})
                        ...
                        # 尝试从文本中提取数字信息
                        import re
                        text = str(context_info) + str(constraints)
                        
                        # 提取GPA
                        gpa_match = re.search(r'(?:gpa|绩点|平均分).*?(\d+\.?\d*)', text, re.IGNORECASE)
                        if gpa_match:
                            gpa = min(4.0, float(gpa_match.group(1)))
                        
                        # 提取排名
                        rank_match = re.search(r'(?:排名|名次).*?(\d+)%', text)
                        if rank_match:
                            ranking = float(rank_match.group(1)) / 100
                        
                        # 提取考试成绩
                        toefl_match = re.search(r'(?:托福|toefl).*?(\d+)', text, re.IGNORECASE)
                        if toefl_match:
                            test_scores['TOEFL'] = float(toefl_match.group(1))
                        
                        ielts_match = re.search(r'(?:雅思|ielts).*?(\d+\.?\d*)', text, re.IGNORECASE)
                        if ielts_match:
                            test_scores['IELTS'] = float(ielts_match.group(1))
                        
                        gre_match = re.search(r'(?:gre).*?(\d+)', text, re.IGNORECASE)
                        if gre_match:
                            test_scores['GRE'] = float(gre_match.group(1))
                        ...
                        academic_profile = AcademicProfile(
                            ...
                            research_experience=0.5,  # 默认中等科研经历
                            publications=0,
                            awards=[],
                            extracurricular=[]
                        )
                        ...
                        # 解析目标学校
                        school_type = SchoolType.STANDARD
                        if any(keyword in option_title for keyword in ['清华', '北大', '985', '顶尖', '名校', 'top']):
                            school_type = SchoolType.TOP
                        elif any(keyword in option_title for keyword in ['211', '重点', '知名']):
                        ...
                        return academic_profile, target_school
                    
                    # 为每个选项提取学业信息
                    logger.info(f"[升学推演] 开始为{len(options)}个选项提取学业信息")
                    options_academic_data = {}
                    for idx, option in enumerate(options):
                        academic_profile, target_school = extract_academic_info(
                            collected_info, 
                            option.get('title', f'选项{idx+1}')
                        )
                        options_academic_data[idx] = {
                            'academic_profile': academic_profile,
                            'target_school': target_school
                        }
                        logger.info(f"[升学推演] 选项{idx+1} [{option.get('title')}] 学业信息提取完成，GPA={academic_profile.current_gpa:.2f}")
                    
                    logger.info(f"[升学推演] 所有选项学业信息提取完成，准备定义推演函数")
                    ...
                    async def simulate_single_education_option(option: dict, option_index: int) -> bool:
                        ...
                        logger.info(f"[升学推演] {option_id} [{option_title}] 开始推演")
                        ...
                        if not await safe_send({
                            ...
                            logger.warning(f"[升学推演] {option_id} 发送option_start失败，连接已断开")
                            return False
                        
                        logger.info(f"[升学推演] {option_id} option_start消息发送成功")
                        ...
                        # 获取该选项的学业数据
                        academic_data = options_academic_data.get(option_index, {})
                        academic_profile = academic_data.get('academic_profile')
                        target_school = academic_data.get('target_school')
                        
                        # 构建教育决策上下文
                        edu_context = EducationDecisionContext(
                            decision_topic=question,
                            current_level=EducationLevel.BACHELOR,  # 本科
                            target_level=EducationLevel.MASTER,  # 硕士
                            academic_profile=academic_profile,
                            target_schools=[target_school],
                            budget=100000.0,
                            preferences={'academic': True, 'career': True}
                        )
                        
                        logger.info(f"[升学推演] {option_id} 教育决策上下文构建完成")
                        
                        # 使用真实算法评估学业竞争力
                        try:
                            competitiveness = edu_engine.evaluate_academic_competitiveness(edu_context)
                            logger.info(f"[升学推演] {option_id} 学业竞争力评估完成: {competitiveness.get('overall_score', 0):.1f}")
                        except Exception as comp_error:
                            logger.warning(f"[升学推演] 学业竞争力评估失败: {comp_error}")
                            import traceback
                            traceback.print_exc()
                            competitiveness = {'overall_score': 70.0, 'dimensions': {}}
                        
                        # 计算录取概率
                        try:
                            admission_result = edu_engine.calculate_admission_probability(
                                target_school,
                                academic_profile
                            )
                            base_admission_prob = admission_result.get('probability', 0.5) * 100
                            logger.info(f"[升学推演] {option_id} 录取概率: {base_admission_prob:.1f}%")
                        except Exception as adm_error:
                            logger.warning(f"[升学推演] 录取概率计算失败: {adm_error}")
                            base_admission_prob = 50.0
                            admission_result = {'probability': 0.5}
                        
                        # 初始化多Agent评估器
                        evaluator = MultiAgentEducationEvaluator(user_id)
                        ...
                        # 初始化Agent
                        logger.info(f"[升学推演] {option_id} 开始初始化5个Agent...")
                        try:
                            # 检查LLM服务状态
                            from backend.llm.llm_service import get_llm_service
                            llm_service = get_llm_service()
                            if llm_service and llm_service.enabled:
                                logger.info(f"[升学推演] {option_id} LLM服务已启用，模型: {getattr(llm_service, 'model_name', 'unknown')}")
                            else:
                                logger.warning(f"[升学推演] {option_id} LLM服务未启用，将使用规则模拟")
                            
                            await evaluator.initialize(agent_context)
                            logger.info(f"[升学推演] {option_id} Agent初始化完成，共{len(evaluator.agents)}个Agent")
                        except Exception as init_error:
                            logger.warning(f"[升学推演] {option_id} Agent初始化失败: {init_error}")
                            import traceback
                            traceback.print_exc()
                        ...
                        for month in range(1, 13):
                            ...
                            # 1. 使用真实算法计算升学评分（随月份进展）
                            progress_factor = month / 12  # 进度因子
                            
                            # 学业表现随时间提升
                            academic_score = competitiveness.get('overall_score', 70.0)
                            current_academic = min(100, academic_score + (progress_factor * 15))
                            
                            # 录取概率随准备提升
                            current_admission_prob = min(95, base_admission_prob + (progress_factor * 25))
                            
                            # 备考进度
                            preparation_progress = min(100, 40 + (month * 5))
                            
                            # 考试准备度
                            exam_readiness = min(100, 45 + (month * 4.5))
                            
                            # 综合得分
                            overall_score = (current_academic * 0.3 + 
                                           current_admission_prob * 0.4 + 
                                           preparation_progress * 0.15 + 
                                           exam_readiness * 0.15)
                            
                            edu_result = {
                                'admission_probability': current_admission_prob,
                                'academic_performance': current_academic,
                                'preparation_progress': preparation_progress,
                                'exam_readiness': exam_readiness,
                                'overall_score': overall_score,
                                'competitiveness_dimensions': competitiveness.get('dimensions', {})
                            }

                            edu_thinking = f"""============================================================
升学决策多维度评估（第{month}月）
============================================================
综合可行性: {edu_result.get('overall_score', 0):.1f}/100 - {get_score_assessment(edu_result.get('overall_score', 0))}

【核心指标评估】
• 学业准备度: {edu_result.get('academic_performance', 0):.1f}/100
• 录取可行性: {edu_result.get('admission_probability', 0):.1f}/100
• 备考完成度: {edu_result.get('preparation_progress', 0):.1f}/100
• 考试就绪度: {edu_result.get('exam_readiness', 0):.1f}/100

【当前状态】
目标学校: {target_school.name} ({target_school.school_type.value})
当前GPA: {academic_profile.current_gpa:.2f} (排名前{academic_profile.ranking_percent*100:.0f}%)

【可行性分析】
基于当前准备度和目标要求，该方案可行性为{edu_result.get('overall_score', 0):.1f}分。
录取可行性{edu_result.get('admission_probability', 0):.1f}分，需要持续提升竞争力。
"""
                            if not await safe_send({
                                "type": "thinking_chunk",
                                "stage": "education_algorithm",
                            ...
                            # 2. 使用真实的多Agent评估（5个Agent并行演化）
                            agent_evaluations = []
                            
                            logger.info(f"[升学推演] {option_id} M{month} 开始5个Agent并行评估，Agent数量: {len(evaluator.agents)}")

                            # 创建5个Agent的并行演化任务
                            async def evolve_single_agent(agent_name: str, agent: Any) -> tuple:
                                """单个Agent的演化任务"""
                                try:
                                    logger.info(f"[升学推演] {option_id} M{month} Agent[{agent_name}] 开始演化，Agent类型: {type(agent).__name__}")
                                    ...
                                    logger.info(f"[升学推演] {option_id} M{month} Agent[{agent_name}] 演化完成，得分: {state.score:.1f}, 状态: {state.status}")
                                    
                                    return (agent_name, state, None)
                                    
                                except Exception as agent_error:
                                    logger.error(f"[升学推演] {option_id} M{month} Agent[{agent_name}] 演化失败: {agent_error}")
                                    import traceback
                                    traceback.print_exc()
                                    return (agent_name, None, agent_error)
                            
                            # 并行执行所有Agent的演化
                            agent_tasks = [
                                evolve_single_agent(agent_name, agent)
                                for agent_name, agent in evaluator.agents.items()
                            ]
                            agent_results = await asyncio.gather(*agent_tasks)
                            
                            # 处理Agent演化结果
                            for agent_name, state, error in agent_results:
                                if error:
                                    logger.error(f"[升学推演] {option_id} M{month} Agent[{agent_name}] 出现错误: {error}")
                                    continue
                                
                                if not state:
                                    logger.warning(f"[升学推演] {option_id} M{month} Agent[{agent_name}] 未返回状态")
                                    continue
                                ...
                                # 生成Agent的详细思考内容
                                agent_display_name = {
                                    'academic_strength': '学业实力Agent',
                                    'admission_probability': '录取概率Agent',
                                    'financial_planning': '财务规划Agent',
                                    'strategy_execution': '策略执行Agent',
                                    'outcome_prediction': '结果预测Agent'
                                }.get(agent_name, agent_name)
                            ...
                            logger.info(f"[升学推演] {option_id} M{month} Agent并行评估完成，共{len(agents_state)}个Agent有状态，{len(agent_evaluations)}个评估结果")
                            ...
                            if agents_state:
                                ...
                                # Agent名称中文映射（调整为评估性术语）
                                agent_name_map = {
                                    'academic_strength': '学业准备度',
                                    'admission_probability': '录取可行性',
                                    'financial_planning': '财务准备度',
                                    'strategy_execution': '执行难度',
                                    'outcome_prediction': '风险评估'
                                }

                                sorted_agents = sorted(agents_state.items(), key=lambda x: x[1].score)
                                weakest = sorted_agents[0] if sorted_agents else None
                                strongest = sorted_agents[-1] if sorted_agents else None

                                summary_parts = [
                                    f"\n{'=' * 50}",
                                    f"【{option_title}】第{month}月 - 多维度综合评估",
                                    f"{'=' * 50}",
                                    f"综合可行性: {overall_score:.1f}/100 | {get_score_assessment(overall_score)}",
                                    f"评估状态: {good_count}个维度良好 | {critical_count}个维度需关注",
                                    f"",
                                    f"【各维度评估】",
                                ]
                                ...
                                for name, state in sorted(agents_state.items(), key=lambda x: -x[1].score):
                                    name_cn = agent_name_map.get(name, name)
                                    status_icon = '✓' if state.status == 'good' else ('⚠' if state.status == 'warning' else '✗')
                                    summary_parts.append(f"  {status_icon} {name_cn}: {state.score:.1f}/100")
                                ...
                                if strongest and weakest:
                                    summary_parts.extend([
                                        ...
                                        f"最弱维度: {agent_name_map.get(weakest[0], weakest[0])} ({weakest[1].score:.1f}分)",
                                        f"",
                                        f"【建议】需重点提升{agent_name_map.get(weakest[0], weakest[0])}，以提高整体可行性",
                                    ])
                                ...
                                if not await safe_send({
                                    ...
                                    "option_title": option_title,
                                    "month": month,
                                    "content": f"\n【{option_title}】第{month}月：等待多维度评估完成..."
                                }):
                                    return False

                            # 4. 生成阶段性评估内容（改为评估性而非预测性）
                            # 将12个月划分为4个关键阶段
                            stage_num = (month - 1) // 3 + 1
                            stage_month_in_stage = ((month - 1) % 3) + 1
                            ...
                            # 根据当前评分生成评估内容
                            overall_score = edu_result.get('overall_score', 50)
                            academic_score = edu_result.get('academic_performance', 50)
                            admission_prob = edu_result.get('admission_probability', 50)
                            
                            # 生成阶段性评估描述
                            if stage_num == 1:
                                if stage_month_in_stage == 1:
                                    event_text = f"【{stage_info['name']}】启动 - 当前学业准备度{academic_score:.0f}分，需重点提升GPA和标准化考试成绩"
                                elif stage_month_in_stage == 2:
                                    event_text = f"【{stage_info['name']}】进行中 - 学业表现{academic_score:.0f}分，建议参加学科竞赛积累优势"
                                else:
                                    event_text = f"【{stage_info['name']}】完成 - 基础准备度{overall_score:.0f}分，进入能力提升阶段"
                            elif stage_num == 2:
                                if stage_month_in_stage == 1:
                                    event_text = f"【{stage_info['name']}】启动 - 录取可行性{admission_prob:.0f}分，需通过项目/竞赛提升竞争力"
                                elif stage_month_in_stage == 2:
                                    event_text = f"【{stage_info['name']}】进行中 - 综合实力{overall_score:.0f}分，建议完成1-2个高质量项目"
                                else:
                                    event_text = f"【{stage_info['name']}】完成 - 能力提升度{overall_score:.0f}分，准备申请材料"
                            elif stage_num == 3:
                                if stage_month_in_stage == 1:
                                    event_text = f"【{stage_info['name']}】启动 - 申请准备度{overall_score:.0f}分，开始撰写文书和准备推荐信"
                                elif stage_month_in_stage == 2:
                                    event_text = f"【{stage_info['name']}】进行中 - 材料完成度{overall_score:.0f}分，建议找专业人士审阅文书"
                                else:
                                    event_text = f"【{stage_info['name']}】完成 - 申请提交完成，录取可行性{admission_prob:.0f}分"
                            else:  # stage_num == 4
                                if stage_month_in_stage == 1:
                                    event_text = f"【{stage_info['name']}】等待中 - 保持学业表现{academic_score:.0f}分，同时准备备选方案"
                                elif stage_month_in_stage == 2:
                                    event_text = f"【{stage_info['name']}】持续跟进 - 综合准备度{overall_score:.0f}分，关注录取动态"
                                else:
                                    event_text = f"【{stage_info['name']}】决策时刻 - 最终可行性评估{overall_score:.0f}分，根据录取结果做出选择"
                            ...
                            # 改为可行性评分（而非概率）
                            feasibility_score = min(0.95, 0.5 + (overall_score / 200))  # 基于综合得分计算可行性
                            ...
                            node = TimelineEvent(
                                ...
                                event=event_text,
                                impact=impact,
                                probability=feasibility_score,  # 改为可行性评分
                                state_before={'month': month - 1},
                                impact_vector=impact,
                            ...
                            logger.info(f"[升学推演] {option_id} M{month} 创建节点: event_id={node.event_id}, parent_event_id={node.parent_event_id}, branch_group={node.branch_group}")
                            ...
                            logger.info(f"[升学推演] {option_id} M{month} 准备发送节点，node_dict['event_id']={node_dict['event_id']}, node_dict['parent_event_id']={node_dict['parent_event_id']}")
                    ...
                    # 并行推演所有选项
                    logger.info(f"[升学推演] 推演函数定义完成，开始并行推演 {len(options)} 个选项")
                    tasks = [simulate_single_education_option(option, i) for i, option in enumerate(options)]
                    logger.info(f"[升学推演] 创建了{len(tasks)}个推演任务，开始执行")
                    results = await asyncio.gather(*tasks, return_exceptions=True)
                    logger.info(f"[升学推演] 所有选项推演完成，结果数量={len(results)}")
                    
                    # 检查是否有异常
                    for i, result in enumerate(results):
                        if isinstance(result, Exception):
                            logger.error(f"[升学推演] 选项{i+1}推演出现异常: {result}")
                            import traceback
                    ...
                    logger.info(f"[升学推演] 准备发送recommendation消息，ws_connected={ws_connected}")
                    if ws_connected:
                        await safe_send({
                            "type": "recommendation",
                            "content": "基于升学教育算法和多Agent评估的综合推荐"
                        })
                        logger.info("[升学推演] recommendation消息发送完成")

                    logger.info(f"[升学推演] 准备发送done消息，ws_connected={ws_connected}")
                    if ws_connected:
                        await safe_send({
                            "type": "done",
                            "simulation_id": session_id,
...
4. 正负事件各半"""
        ...
        logger.error(f"决策模拟 WebSocket 失败: {e}", exc_info=True)
        import traceback
        traceback.print_exc()
        # 只在连接仍然活跃时尝试发送错误消息
        if ws_connected:
            try:
                ...
                logger.warning(f"发送错误消息失败（连接可能已断开）: {send_err}")
