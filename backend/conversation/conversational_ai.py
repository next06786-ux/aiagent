...
def get_kg_rag_context(user_id: str, query: str, max_nodes: int = 10) -> Dict[str, Any]:
    ...
    # 使用全局缓存字典
    if not hasattr(get_kg_rag_context, '_cache'):
        get_kg_rag_context._cache = {}
    
    # 检查缓存（5分钟有效期）
    import time
    if cache_key in get_kg_rag_context._cache:
        cached_data, timestamp = get_kg_rag_context._cache[cache_key]
    ...
    try:
        ...
        result = {
            ...
            "influence_summary": _calc_influence(context.results),
            "reasoning": f"使用{context.strategy_used.value}策略"
        }
        
        # 缓存结果
        _kg_context_cache[cache_key] = (result, time.time())
        return result
        
    except Exception as e:
        print(f"获取知识图谱上下文失败: {e}")
        return {"context_text": "", "nodes": [], "influence_summary": {}, "reasoning": ""}


def _format_context(context) -> str:
    """格式化上下文"""
    lines = []
    for r in context.results[:10]:
        lines.append(f"- [{r.node_type}] {r.content[:50]}...")
    return "\n".join(lines)


def _calc_influence(results) -> Dict:
    """计算影响力"""
    summary = {}
    for r in results:
        cat = r.category or 'unknown'
        summary[cat] = summary.get(cat, 0) + r.score
...
class ConversationalAI:
    ...
    async def chat(
        ...
        Args:
            ...
            enable_function_calling: 是否启用函数调用（默认True）
        ...
        if enable_function_calling:
            function_result = await self._try_function_calling(user_id, message)
            if function_result:
                # 函数调用成功，返回结果
...
4. 如果信息不足，基于通用知识回答"""},
    ...
    async def _try_function_calling(self, user_id: str, message: str) -> Optional[str]:
        """
        尝试函数调用
        使用 LLM 判断是否需要调用函数，并执行调用
        """
        from backend.conversation.function_calling import function_registry
        from backend.llm.llm_service import get_llm_service
        ...
        try:
            ...
            prompt = f"""你是一个智能助手，可以调用系统函数来帮助用户。
...
注意：
1. 当用户要求"安排日程"、"生成日程"、"规划时间"时，调用 generate_schedule
2. 当用户询问"历史日程"、"过去的安排"时，调用 query_schedule
3. 当用户询问"人际关系"、"朋友"、"家人"时，调用 query_relationships
4. user_id 参数使用: "{user_id}"
5. 如果用户说"今天"，date 参数使用当天日期
...
只返回JSON，不要其他内容。"""
            ...
            if not result.get("need_function_call"):
                return None
            ...
            print(f"[Function Calling] 检测到函数调用需求")
            print(f"  函数: {function_name}")
            print(f"  参数: {arguments}")
            print(f"  原因: {reasoning}")
            
            # 调用函数
            call_result = await function_registry.call_function(function_name, arguments)
            
            if call_result.get("success"):
                # 生成友好的回复
                function_output = call_result.get("result", {})
                
                # 根据函数类型生成不同的回复
                if function_name == "generate_schedule":
                    return f"好的，我正在为你生成日程安排！\n\n{function_output.get('message', '')}\n\n你可以在「智能日程」页面查看生成进度和结果。"
                elif function_name == "query_schedule":
                    schedules = function_output.get("schedules", [])
                    if schedules:
                        return f"找到了 {len(schedules)} 条相关日程记录。你可以在智能日程页面查看详情。"
                    else:
                        return "暂时没有找到相关的日程记录。"
                elif function_name == "query_relationships":
                    relationships = function_output.get("relationships", [])
                    if relationships:
                        rel_list = "\n".join([
                            f"- {r['person']} ({r['type']})"
                            for r in relationships[:5]
                        ])
                        return f"你的人际关系网络中有 {len(relationships)} 个联系人：\n{rel_list}\n\n{'...' if len(relationships) > 5 else ''}你可以在知识星图页面查看完整的关系网络。"
                    else:
                        return "暂时没有找到人际关系数据。"
                else:
                    return f"已执行 {function_name}，结果：{json.dumps(function_output, ensure_ascii=False)}"
            else:
                error = call_result.get("error", "未知错误")
                print(f"[Function Calling] 函数调用失败: {error}")
                return f"抱歉，执行操作时遇到了问题：{error}"
                
        except Exception as e:
            import traceback
            print(f"[Function Calling] 函数调用系统错误: {e}")
    ...
    async def chat_stream(
        ...
        """
        流式对话接口 - 默认使用知识图谱感知RAG + 智能导航 + 函数调用
        ...
        try:
            ...
            if enable_function_calling:
                ...
                if function_result:
                    # 函数调用成功，流式返回结果
                    yield {"type": "function_call", "content": "正在执行操作..."}
                    yield {"type": "content", "content": function_result}
