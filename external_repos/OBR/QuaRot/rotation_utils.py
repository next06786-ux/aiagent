...
def fuse_layer_norms(model):
    ...
    if model_type == model_utils.LLAMA_MODEL:
        norm_type = transformers.models.llama.modeling_llama.LlamaRMSNorm
    elif model_type == model_utils.QWEN_MODEL:
        norm_type = transformers.models.qwen2.modeling_qwen2.Qwen2RMSNorm
    else:
...
def rotate_ov_proj(layer, model_type, head_num, head_dim):
    ...
    apply_exact_had_to_linear(v_proj, had_dim=head_dim, output=True) # 注意这里有个分head操作, 因为不同head间肯定是不能混用的. 相当于head内部的旋转
    apply_exact_had_to_linear(o_proj, had_dim=-1, output=False)  # 真的好神奇, 对V明明是每个head内部的旋转, 到了O这里直接一步整体旋转就cancel掉了
...
def rotate_model(model, args):
    ...
    for idx, layer in enumerate(tqdm.tqdm(layers, unit="layer", desc="Rotating")):
        rotate_attention_inputs(layers[idx], Q, model_type) # QKV
        rotate_attention_output(layers[idx], Q, model_type) # O
        rotate_mlp_input(layers[idx], Q, model_type) # up gate
...
class QKRotationWrapper(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        if self.k_groupsize == -1: #token-wise quantization
            token_wise_k = k.transpose(1, 2).reshape(bsz*seq_len, -1)
