...
def parser_gen():
    ...
    parser.add_argument('--obr_alpha', type=float, default=0.5, help='The Group Partition ratio in OBR_GPTQ')
    parser.add_argument('--obr_rtn', action='store_true', default=False,
                        help='Quantize the weights using RtN. If the w_bits < 16 and this flag is not set, we use GPTQ')
    # Prune Parameters in obr
    parser.add_argument('--sparsity_ratio', type=float, default=0.5,
                        help='Sparsity ratio in unstructured pruning')
    parser.add_argument('--prune_n', type=float, default=0,
                        help='The N in N:M structured pruning, only support 2:4 and 4:8!')
    parser.add_argument('--prune_m', type=float, default=0,
                        help='The M in N:M structured pruning, only support 2:4 and 4:8!')
    ...
    parser.add_argument("--load_qmodel_path", type=str, default="", help="Path to load the quantized model")
    parser.add_argument("--save_qmodel_path", type=str, default="", help="Path to save the quantized model")

    # LM Eval Arguments
    parser.add_argument("--ppl_eval", action="store_true", help="Evaluate the model on LM Eval tasks.")
