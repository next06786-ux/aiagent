...
from eval_utils.prune_utils import wanda_pruning
...
def obr_rtn_quantization(W, quantizer):
    q, q_int_weight, q_scale = quantizer.fake_quantize(W)
    return q
...
def obr_gptq_quantization(W, quantizer, XTX, blocksize, W_mask, obr_alpha):
    ...
    for i1 in range(0, columns, blocksize):
        ...
        for i in range(count):
            ...
            d = Hinv1[i, i]
            w = w
            q, int_weight, scale = quantizer.fake_quantize(w.unsqueeze(1))
            q = q.flatten()
            q = q * W_mask[:, col_idx]
...
class OBR_Wrapper:
    ...
    def optimal_brain_restoration(self, percdamp=.01,sparsity_ratio = 0.5, prune_n=0, prune_m = 0, obr_rtn=False,obr_alpha=0.5, blocksize=128,):
        ...
        #########################################
            ...
            q1 = self.quantizer.fake_quantize(W[:, :int(obr_alpha*W.shape[1])])[0]
    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                gptq[name].quantizer = quant_utils.WeightQuantizer() # RTN quantizer
                gptq[name].name = name
